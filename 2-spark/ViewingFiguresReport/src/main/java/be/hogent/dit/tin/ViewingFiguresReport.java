...
public class ViewingFiguresReport {
	...
	SparkSession spark = SparkSession.builder().appName("ViewingFigures").master("local[*]").getOrCreate();

	/*
	 * 
	 */
	public Dataset<Row> readChapters(String pathname_data, String pathname_header) {
		Dataset<Row> chapters_data = this.spark.read().csv(pathname_data);
		Dataset<Row> chapters_header = spark.read().option("header", true).csv(pathname_header);
	...
	public Dataset<Row> readViews(String pathname_data, String pathname_header) {
		Dataset<Row> chapters_data = this.spark.read().csv(pathname_data);
		Dataset<Row> views_header = spark.read().option("header", true).csv(pathname_header);
	...
	public Dataset<Row> readTitles(String pathname_data, String pathname_header) {
		Dataset<Row> titles_data = this.spark.read().csv(pathname_data);
		Dataset<Row> titles_header = spark.read().option("header", true).csv(pathname_header);
	...
	public Dataset<Row> count_viewed_chapters(Dataset<Row> chapters, Dataset<Row> views, Dataset<Row> titles) {
		Column joinexpr1 = views.col("chapterId")
				.equalTo(chapters.col("chapterId"));

		Column joinexpr2 = chapters.col("courseId").equalTo(titles.col("courseId"));
		
		return views.join(chapters, joinexpr1).join(titles, joinexpr2).groupBy(col("userId"), titles.col("courseId"))
				.count();
	}
	...
	public Dataset<Row> calculatepercentage(Dataset<Row> totalchapters, Dataset<Row> totalviews) {
		...
		return totalchapters
				.join(totalviews, joinexpr)
				.select(totalchapters.col("courseId"), totalviews.col("count"), totalchapters.col("count"),
						col("title"))
	...
	public static void main(String[] args) {
		...
		vfr_controller.spark.sparkContext().setLogLevel("ERROR");
		
		Dataset<Row> chapters = vfr_controller.readChapters("src/main/resources/chapters.csv",
				"src/main/resources/chapters-header.csv");
		Dataset<Row> views = vfr_controller.readViews("src/main/resources/views-1.csv",
				"src/main/resources/viewsHeader.csv");
		Dataset<Row> titles = vfr_controller.readTitles("src/main/resources/titles.csv",
				"src/main/resources/titles-header.csv");
		...
		totalWithPercent.select(totalWithPercent.col("courseId"), col("percentage"), col("title"))
				.withColumn("score", functions.callUDF("berekenScore", col("percentage")))
				.drop(col("percentage"))
				.orderBy(desc("score")).show();
